...
public class PlayerSvc {
	...
	@RequestMapping(value="player/auth", method=RequestMethod.POST)
	public @ResponseBody Player authenticatePlayer(@RequestParam("username") String username, @RequestParam("passwordHash") String passwordHash, HttpServletResponse response){
		try {
			Player p = players.findByUsername(username);
			if (p.getPasswordHash()== passwordHash){
